// Composite matcher: chains two Gomega matchers together so a single
// Expect(...).To call can assert multiple action-call expectations on the
// same triggering function. Build chains via NewCompositeMatcher or the
// (*compositeMatcher).And helper.
package matcher
