// ActionPerformerImpl is the generic execution engine that drives an Action
// through the lifecycle (IsAllowed → IsEnabled → IsValid → Perform →
// AfterCommit). It is constructed via ba.New[A](ctx, action) and exposes
// Perform, Try, IsPerformable, As, and AsSystem.
package ba
